...
ifeq ($(BOXMODEL)-$(FLASH_SIZE),$(filter $(BOXMODEL)-$(FLASH_SIZE),e4hdultra-8gb))
  EMMC_IMAGE_SIZE := $$(( $(EMMC_IMAGE_SIZE)*2 ))
endif
...
ifeq ($(BOXMODEL)-$(FLASH_SIZE),$(filter $(BOXMODEL)-$(FLASH_SIZE),e4hdultra-8gb))
  SWAP_PARTITION_SIZE := $$(( $(SWAP_PARTITION_SIZE)*2 ))
endif

# just needed to calculate MULTI_ROOTFS_PARTITION_SIZE; STORAGE_PARTITION uses remaining flash memory
STORAGE_PARTITION_SIZE = $(SWAP_PARTITION_SIZE)
...
ifeq ($(IMAGE_LAYOUT),subdirboot)
  ...
  ifeq ($(BOXMODEL)-$(FLASH_SIZE),$(filter $(BOXMODEL)-$(FLASH_SIZE),e4hdultra-8gb))
    ROOTFS_PARTITION_SIZE := $$(( $(ROOTFS_PARTITION_SIZE)*2 ))
  endif
...
flash-image-vuplus-multi: | $(IMAGE_DIR)
	...
	cp $(BUILD_DIR)/$(VMLINUZ_INITRD_FILE) $(IMAGE_BUILD_DIR)/$(IMAGE_SUBDIR)/initrd_auto.bin
