...
$(TARGET_sysconfdir)/init.d/functions:
	$(INSTALL_DATA) -D $(INIT_SCRIPTS_FILES_DIR)/init.functions $(@)

$(TARGET_sysconfdir)/init.d/camd:
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/camd.init $(@)
	$(UPDATE-RC.D) $(@F) defaults 98 01

$(TARGET_sysconfdir)/init.d/camd_datefix:
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/camd_datefix.init $(@)

$(TARGET_sysconfdir)/init.d/coredump:
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/coredump.init $(@)
	$(UPDATE-RC.D) $(@F) start 40 S .

$(TARGET_sysconfdir)/init.d/custom-poweroff:
ifeq ($(BOXTYPE),coolstream)
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/custom-poweroff.init $(@)
...
$(TARGET_sysconfdir)/init.d/proc:
ifeq ($(BOXMODEL),$(filter $(BOXMODEL),hd51 bre2ze4k h7 e4hdultra protek4k hd60 hd61 multibox multiboxse vusolo4k vuduo4k vuduo4kse vuultimo4k vuzero4k vuuno4k vuuno4kse vuduo))
...
$(TARGET_sysconfdir)/init.d/sys_update.sh:
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/sys_update.sh $(@)

$(TARGET_sysconfdir)/init.d/sendsigs:
ifeq ($(BOXMODEL),$(filter $(BOXMODEL),hd60 hd61 multibox multiboxse))
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/sendsigs.init $(@)
	$(UPDATE-RC.D) $(@F) start 85 0 .
endif

$(TARGET_sysconfdir)/init.d/umountfs:
ifeq ($(BOXMODEL),$(filter $(BOXMODEL),hd60 hd61 multibox multiboxse))
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/umountfs.init $(@)
	$(UPDATE-RC.D) $(@F) start 86 0 .
endif

$(TARGET_sysconfdir)/init.d/suspend:
ifeq ($(BOXMODEL),$(filter $(BOXMODEL),hd60 hd61 multibox multiboxse))
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/suspend.init $(@)
	$(UPDATE-RC.D) $(@F) start 89 0 .
endif

$(TARGET_sysconfdir)/init.d/user-initscripts:
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/user-initscripts.init $(@)
	$(UPDATE-RC.D) $(@F) defaults 98 01

$(TARGET_sysconfdir)/init.d/stb_update.sh:
ifeq ($(BOXSERIES),$(filter $(BOXSERIES),hd1 hd2))
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/stb_update_$(BOXSERIES).sh $(@)
endif

$(TARGET_sysconfdir)/init.d/rootx-mount:
ifeq ($(BOXSERIES),$(filter $(BOXSERIES),hd2))
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/rootx-mount.init $(@)
	$(UPDATE-RC.D) $(@F) defaults 01 98
endif

$(TARGET_sysconfdir)/init.d/var_mount.sh:
ifeq ($(PERSISTENT_VAR_PARTITION),yes)
	$(INSTALL_EXEC) -D $(INIT_SCRIPTS_FILES_DIR)/var_mount.sh $(@)
endif
