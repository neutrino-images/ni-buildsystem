...
NEUTRINO_PLUGINS_SRC_DIR = $(SOURCE_DIR)/$(NEUTRINO_PLUGINS_DIR)
NEUTRINO_PLUGINS_OBJ_DIR = $(BUILD_DIR)/$(NEUTRINO_PLUGINS_DIR)-obj

NEUTRINO_PLUGINS_MISSING = $(wildcard $(NEUTRINO_PLUGINS_SRC_DIR)/missing)
...
define NEUTRINO_PLUGINS_AUTOGEN_SH
	...
	$(CD) $(NEUTRINO_PLUGINS_OBJ_DIR); \
		$(NEUTRINO_PLUGINS_SRC_DIR)/autogen.sh
endef
...
define NEUTRINO_PLUGINS_CONFIGURE_CMDS
	$(CD) $(NEUTRINO_PLUGINS_OBJ_DIR); \
		$($(PKG)_CONF_ENV) \
		$(NEUTRINO_PLUGINS_SRC_DIR)/configure \
			$($(PKG)_CONF_OPTS)
endef
...
neutrino-plugins: | $(TARGET_DIR)
	$(call autotools-package,$(PKG_NO_EXTRACT) $(PKG_NO_PATCHES) $(if $(NEUTRINO_PLUGINS_MISSING),$(if $(NEUTRINO_PLUGINS_CONFIG_STATUS),$(PKG_NO_CONFIGURE))))
