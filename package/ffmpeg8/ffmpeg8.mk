...
FFMPEG8_VERSION = 8.0.1
FFMPEG8_DIR = ffmpeg-$(FFMPEG8_VERSION)
FFMPEG8_SOURCE = ffmpeg-$(FFMPEG8_VERSION).tar.xz
FFMPEG8_SITE = http://www.ffmpeg.org/releases

FFMPEG8_DEPENDENCIES = openssl zlib bzip2 harfbuzz freetype rtmpdump libbluray libass libxml2 libvorbis alsa-lib
...
ifeq ($(BS_PACKAGE_FFMPEG8_FFPLAY),y)
  FFMPEG8_DEPENDENCIES += sdl2
  FFMPEG8_CONF_OPTS += --enable-ffplay
  FFMPEG8_CONF_ENV += SDL_CONFIG=$(HOST_DIR)/bin/sdl2-config
else
  FFMPEG8_CONF_OPTS += --disable-ffplay
endif

ifeq ($(BS_PACKAGE_FFMPEG8_FFPROBE),y)
  FFMPEG8_CONF_OPTS += --enable-ffprobe
else
  FFMPEG8_CONF_OPTS += --disable-ffprobe
endif

FFMPEG8_DEPENDENCIES += lame
FFMPEG8_CONF_OPTS += --enable-libmp3lame
